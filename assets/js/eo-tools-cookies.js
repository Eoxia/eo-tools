			...
			Object.keys(this.categories).forEach(cat => {
				...
				if (registry[cat] && registry[cat].length > 0) {
					...
					registry[cat].forEach(cookie => {
						cookieListHtml += `<tr><td style="padding: 8px 5px 8px 0; border-bottom: 1px solid #e2e8f0;"><code>${cookie.name}</code></td><td style="padding: 8px 5px; border-bottom: 1px solid #e2e8f0;">${cookie.domain || ''}</td><td style="padding: 8px 5px; border-bottom: 1px solid #e2e8f0;">${cookie.duration} jours</td><td style="padding: 8px 0 8px 5px; border-bottom: 1px solid #e2e8f0;">${cookie.description}</td></tr>`;
					});
